...
int fh_jpeg_load(aml_dec_para_t* para , aml_image_info_t* image)
{
	...
	//{
		lb = malloc(3*px);
		if(!lb) goto exit;
		//lb=(*ciptr->mem->alloc_small)((j_common_ptr) ciptr,JPOOL_PERMANENT,3*px);
		if(para->colormode == 2 ) {
	...
	image->depth = 32;
	if(para->mode == 4) {
		image->width = para->dest_w;
		image->height = para->dest_h;
		
	} else {
		image->width = x;
		image->height = y;
	}
	image->nbytes = image->bytes_per_line * image->height;  
