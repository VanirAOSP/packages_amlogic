...
//unsigned char too_large_width_ratio = 0;
int sMode;
...
{
	int i = 0;
	LOGI("last fd_amport is (%d).\n",fd_amport);  
	fd_amport = open(FILE_NAME_AMPORT, O_WRONLY);
	if(fd_amport<0) {
		LOGI("error:hwjpeg initing,can't access %s.\n",FILE_NAME_AMPORT);
		LOGI(" error no %d\n",errno);
		return -1;
	}
	...
	while(access(FILE_NAME_JPEGDEC, R_OK|W_OK)) { //waitting for device created.
		...
		i++;
		if(i>20)  {
			LOGI("hw jpeg init decoder error---hw jpeg device access error\n");	
			return -1;
		}
	}
	return 0;
...
unsigned int  ImgFormat2Ge2dFormat(int img_format)
{
	...
	case COLOR_S32_ABGR:
		...
		break;
	default:
		LOGI("blit_32(): Image format %d not supported!", img_format);
		format = GE2D_FORMAT_S32_ARGB;
		break;
	}
	 return format;
...
int clear_plane(int index,jpegdec_config_t *config,int format)
{
	...
	if(fd_ge2d<0) {
		LOGI("can't open framebuffer device" );
		goto exit;
	}
	LOGI("start clean plane buffer %d!!!!!\n", index);
...
exit:
	...
	LOGI("finish clean plane buffer %d!!!!!\n", index);
	return 0;
}
...
int compute_keep_ratio(jpeg_data_t  *jpeg_data,jpegdec_config_t *config)
{
	...
	config->dec_x = 0; 
	config->dec_y = 0;
	if(decoder_opt && (jpeg_data->info.width > 1280)) {
		config->dec_w = target_width * 4;
		config->dec_h = target_height * 4;
	} else {
...
int compute_keep_ratio_by_expanding(jpeg_data_t  *jpeg_data,jpegdec_config_t *config)
{
	...
	if((image_width <= frame_width)&&(image_height <= frame_height)){        
		...
		LOGI("crop function disable %d*%d!!!!!\n", image_width, image_height);          
		return -1;
	}
	return 0;
}
int rebuild_jpg_config_para(jpeg_data_t  *jpeg_data,jpegdec_config_t *config)
{
	...
	if(config->dec_h<2) {
		LOGI("too small to decode with hwjpeg decoder.\n");
		return -1;
	}
	return 0;
}
...
int read_jpeg_data(FILE* fd,jpeg_data_t  *jpeg_data,int op_max,jpegdec_config_t *config)
{
	...
	while(op_step < op_max) {
		...
		if (decState & JPEGDEC_STAT_UNSUPPORT) {
			LOGI("jpegdec unsupported format\n");
			break;
		}
		...
		if((!file_read_end)&&(vb_info.status.data_len < ((4*vb_info.status.size)/5))&&(decState & JPEGDEC_STAT_WAIT_DATA)){
			...
			if(read_num<0)
			{
				LOGI("can't read data from jpeg device");  
				result= 0;
				break;
			}
		...
		case DEC_STAT_INFO:
			if (decState & JPEGDEC_STAT_INFO_READY) {
				...
				if(wait_info_count > 100){
					...
					//LOGI("current timeout is %d!!!\n",wait_timeout);
				}
				if(wait_timeout > 1) {
					op_step = op_max;
					LOGI("timeout for get jpeg info!!!\n");	
					result =0;
					break;
				}
				//LOGI("in jpeg decoding process\n");
				result =0;
			}
			break;
		...
		if((time_poll - time_start) > 3){
			op_step = op_max;
			LOGI("it's a corrupted jpeg\n");
			result =0;
		}
	}
	LOGI("decoder exit\n");
	LOGI("total read bytes is %d",total_size);
...
{  
	...
	if(!(JPEGDEC_STAT_DONE & 
			read_jpeg_data(fd, &jpeg_data, DEC_STAT_MAX, &config))) {    
		LOGI("can't decode jpg pic\n");
		goto exit;
	} 
	...
	if(!input_image_info) {
		LOGI("amljpeg: info mem get error\n");
		goto exit;
	}

	LOGI("deocde jpg pic completed\n");
	...
	LOGI("start ge2d image format convert!!!!!\n");
	ge2d_config.src_dst_type = ALLOC_ALLOC;
	...
	{
		LOGI("can't get proper ge2d format\n" );
		goto exit;
	}
	...
	if(ssMode == 4){
	    input_image_info->width = op_para.dst_rect.w;
	    input_image_info->height = op_para.dst_rect.h;
	}
	else{
...
aml_image_info_t* read_jpeg_image(aml_dec_para_t* para)
{
	...
	{ 
		...
		if(!output_image_info->data){
			LOGI("err alloc output_image_info->data\n");
			goto umap_exit;    
		}
...
aml_image_info_t* read_2_frame_image(aml_dec_para_t* para)
{
	...
	if(fseek(fd,para->image_3d_info.frame[0],SEEK_SET)) {
		LOGI("amljpeg:source file:%s seek error\n",para->fn);
		goto exit;   
	}
	if(fd_jpegdec>=0) close(fd_jpegdec);
	fd_jpegdec= open(FILE_NAME_JPEGDEC, O_RDWR); 
	if(fd_jpegdec <0 ){
		LOGI("open amjpec device error %d \n",errno);
		goto exit; 
	}
	output_image_info = (aml_image_info_t*)malloc(sizeof(aml_image_info_t));
	if(!output_image_info) {
