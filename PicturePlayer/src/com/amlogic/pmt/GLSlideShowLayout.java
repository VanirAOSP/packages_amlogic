...
public class GLSlideShowLayout extends GLBaseLayout implements
	...
	public final static float T = 1.8f; // top
	public final static float B = -1.8f; // bottom
	final static float N = 0.00001f; // near
	final static float S = 0.00001f; // small
	final static float F = 2.0f; // far

	final static int BCRows = 18 + 1;
	...
	private boolean rotateStatus = false;
	...
	private void switchSlot() {
		...
		if(status == 0){
			if (true == DebugConfig.debug)
				Log.d("GLSlide", "switchSlot return");
			return;
		}
		synchronized (slots) {
			Message msg = new Message();
			msg.what = 0;
			handlerGetStatus.sendMessage(msg);
			if (getSize() < 2)
				return;
			/*
			 * int sm = currentSwitchMode; if (sm < 0 || sm >=
			 * SwitchAnimParam.length) { sm = (int) (Math.random() *
			 * (SwitchAnimParam.length - 1)) + 1; }
			 */
			...
			am0.setScale(SwitchAnimParam[sm][0][6], SwitchAnimParam[sm][0][7],
					SwitchAnimParam[sm][0][8], SwitchAnimParam[sm][0][9],
					SwitchAnimParam[sm][0][10], SwitchAnimParam[sm][0][11]);
			...
			am1.setScale(SwitchAnimParam[sm][1][6], SwitchAnimParam[sm][1][7],
					SwitchAnimParam[sm][1][8], SwitchAnimParam[sm][1][9],
					SwitchAnimParam[sm][1][10], SwitchAnimParam[sm][1][11]);
		...
		if (Menucontrol.initState != null) {
			for (int i = 0; i < Menucontrol.initState.size(); i++) {
				if (Menucontrol.initState.get(i).contains(
						"shortcut_picture_rotate_")) {
					if (DebugConfig.debug == true)
						Log.d("GLSlideShowLayout", ">>>>>>>>>>Remove "
								+ Menucontrol.initState.get(i));
	...
	public void zoom(float fScale) {
		synchronized (slots) {
			...
			PMTTransformAnimation am1 = new PMTTransformAnimation(slot1,
					new DecelerateInterpolator(), SWITCH_TIMING, 0);
			am1.setScale(currentScale, currentScale, 1.0f, fScale, fScale, 1.0f);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		// if(DebugConfig.debug == true)
		if (true == DebugConfig.debug)
			Log.d("GLSlideShowLayout", "onKeyDown(" + keyCode + ")");
		switch (keyCode) {
	...
	public void setPosXY(float posX, float posY) {
		...
		getWH();
		if (currentScale > defaultScale) {
			if ((posY > 0 && this.posY >= ((currentScale / defaultScale) * h
					/ 2 - 0.9f))
					|| (posY < 0 && (-this.posY) >= ((currentScale / defaultScale)
							* h / 2 - 0.9f))) {
			} else if ((currentScale / defaultScale) * h > 1.8f) {
				this.posY = this.posY + posY;
				flag = true;
			}
			if ((posX > 0 && this.posX >= ((currentScale / defaultScale) * w
					/ 2 - 1.6f))
					|| (posX < 0 && (-this.posX) >= ((currentScale / defaultScale)
							* w / 2 - 1.6f))) {
			} else if ((currentScale / defaultScale) * w > 3.2f) {
				this.posX = this.posX + posX;
				flag = true;
			}
			if (flag == true) {
				if ((currentScale / defaultScale) * h > 1.8f) {
					if (posY > 0
							&& this.posY > ((currentScale / defaultScale) * h
									/ 2 - 0.9f))
						this.posY = (currentScale / defaultScale) * h / 2
								- 0.9f;
					else if (posY < 0
							&& (-this.posY) > ((currentScale / defaultScale)
									* h / 2 - 0.9f))
						this.posY = 0.9f - (currentScale / defaultScale) * h
								/ 2;
				}

				if ((currentScale / defaultScale) * w > 3.2f) {
					if (posX > 0
							&& this.posX > ((currentScale / defaultScale) * w
									/ 2 - 1.6f))
						this.posX = (currentScale / defaultScale) * w / 2
								- 1.6f;
					else if (posX < 0
							&& (-this.posX) > ((currentScale / defaultScale)
									* w / 2 - 1.6f))
						this.posX = 1.6f - (currentScale / defaultScale) * w
								/ 2;
				}
			}
			if (flag == true)
				setXYPosition(this.posX, this.posY);

		}
		if (DebugConfig.debug == true)
			Log.d("GLS", "defaultScale:" + defaultScale + "currentScale:"
					+ currentScale + "currentX:" + currentX + "currentY:"
					+ currentY);
	}
	...
	public void setScale(float fScale) {
		if (fScale < 0) {
			...
			float posX = 0;
			float posY = 0;
			if (((this.posX + 1.6f - w / 2 * (currentScale / defaultScale)) > 0)
					&& this.posX > 0) {
				posX = fScale * currentScale * 2;
				flag = true;
			} else if (((-this.posX + 1.6f - w / 2
			...
			if (((this.posY + 0.9f - h / 2 * (currentScale / defaultScale)) > 0)
					&& this.posY > 0) {
				posY = fScale * currentScale * 2;
				flag = true;
			} else if (((-this.posY + 0.9f - h / 2
					* (currentScale / defaultScale)) > 0)
					&& this.posY < 0) {
				posY = -fScale * currentScale * 2;
				flag = true;
		...
		cScale = currentScale;
		if ((currentScale > defaultScale * 16.0f && fScale > 0)
				|| (currentScale < defaultScale / 16.0f && fScale < 0))
			return;
		cScale = cScale + fScale;
		zoom(cScale);
		if (DebugConfig.debug == true)
			Log.d("GLS", "currentScale:" + currentScale + "currentX:"
					+ currentX + "currentY:" + currentY);
	}
...
//		setCamera(gl);
		synchronized (slots) {
			if (stateAutoPlay == STATE_AUTOPLAY_LOADTEXTURE) {
				Message msg = new Message();
                        	msg.what = 0;
                        	handlerGetStatus2.sendMessage(msg);
		...
		if ((test > 0 && test < 100) || test == -2) {
			Message vitioMsg = new Message();
			Bundle vitioData = new Bundle();
			vitioData.putFloat("vitioBundle", test);
			vitioMsg.setData(vitioData);
			handlerGetVitio.sendMessage(vitioMsg);
			test = 0;
			Message msg = new Message();
			msg.what = 1;
			handlerGetStatus.sendMessage(msg);
	...
	public Handler handlerGetVitio = new Handler() {
		public void handleMessage(Message msg) {
			super.handleMessage(msg);
			Bundle b = msg.getData();
			vitio = b.getFloat("vitioBundle");
			if (vitio == -2) {
		//		if (DebugConfig.debug == true)
					Log.d("GLSlide", "return -2 is a wrong picture");
			}
		}
	};

        public Handler handlerGetStatus2 = new Handler() {
                public void handleMessage(Message msg) {
			switch (msg.what) {
                        case 0:
                                status2 = 0;
                        ...
                        super.handleMessage(msg);
                //              if (DebugConfig.debug == true)
                                        Log.d("GLSlide", "handlerGetStatus2:" + status2);
                }
        };
	public Handler handlerGetStatus = new Handler() {
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case 0:
				status = 0;
			...
			super.handleMessage(msg);
//			if (true == DebugConfig.debug)
				Log.d("GLSlide", "handlerGetStatus:" + status);
		}
	};
	...
	public void CallbackMenuState(String... State) {
		...
		Log.d("GLSlide", "CallbackMenuStatus return status:" + status);
		...
		synchronized (slots) {
			...
			if (State[1].equals("shortcut_common_stop_")) {
				...
				if (!currentRandomPlay)
					this.dataProvider
							.setSwitchMode(DataProvider.playmode_normal);
				...
				synchronized (slots) {
					if (autoPlayflag == true) {
						...
						if (name != null) {
							...
							Log.d("GLSlide", "AutoPlay loadTexture" + status);
						}
					}
				}
				break;
