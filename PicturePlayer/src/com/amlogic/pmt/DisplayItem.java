...
//	float x=0;
   public synchronized float drawItem(GL10 gl){
	float w=0;
	float h=0;
	float ratio = 0;
		if(isVisible){		
		    if(fileflag == true && bitmap == null){
			ratio = (float)widhei[0] / widhei[1];
			if(ratio >= 1.7778){
			    w= 3.2f;
			    h = 3.2f / ratio;
			}else{
			...
			paintBoardVertex = null;
			paintBoardIndex = null;
			paintBoardColor = null;
			paintBoardTexCoord = null;
			this.setResolution(w,h,32,18);
			fileflag = false;
		    }
			///////////////////////////////////////////////////////////////////////////////////////////////////////
		    if(!(!isThumbMode() && imageTexID<0 && fileName != null && fileName != "")){
			gl.glPushMatrix();
			glPoseDraw(gl);
		    ...
		    if(isThumbMode()){
				...
				if(imageTexID<0){
					if(bitmap != null){
						...
						imageTexID = TextureManager.loadTextureFile(gl,pathName+fileName,widhei);
						if (-2 == imageTexID)
							return -2;
						ratio = (float)widhei[0] / widhei[1];
						if(ratio >= 1.7778){
	...
	public void setResolution(float w, float h, int collums, int rows){
		...
		paintBoardRow = (rows<=18)?((rows/2)*2): 18;
