...
//	private static ArrayList<Integer> deledTexList = new ArrayList();
	...
	static int loadTextureFile(GL10 gl, String fileName, int[] widhei){
		Bitmap bmp = null;
		File fl = new File(fileName);
		...
		String PlayFormat[] = {"jpg","jpeg","png","bmp","gif","mpo","tif","tiff"};
		...
		if(bmp == null){	//software decode.
		    ...
		    Log.d("TextureFileLoad", "return bmp null"); 
		}else{
		//	if(DebugConfig.debug == true)
		...
		if(bmp == null){
			if(DebugConfig.debug == true)
				Log.i("TextureManager", "<-- loadTextureFile(" + fileName + ") -2 (bmp==null)");
			return -2;
		}
		widhei[0] = bmp.getWidth();
                widhei[1] = bmp.getHeight();
		int texID = loadTexture(gl, bmp, false);
		bmp.recycle();
		if(DebugConfig.debug == true)
