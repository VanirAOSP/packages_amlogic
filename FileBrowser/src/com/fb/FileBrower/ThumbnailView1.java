...
public class ThumbnailView1 extends Activity{
	...
	private boolean mSearchedListLoaded = false;
	...
	protected static final int SEARCH_DIALOG_ID=5;
    ...
    private ThumbnailAdapter1 getFileListAdapterSorted(String path, String sort_type) {
		lv_search_key = null; // leaving searched list screen ,clear search flag
    	if (path.equals(ROOT_PATH)) {
            return new ThumbnailAdapter1(ThumbnailView1.this,
            		getDeviceListData(),
            		R.layout.gridview_item,        		
...
//        m_storagemgr.unregisterListener(mListener);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mProgressHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
				...
				case 11:	//destination dir is sub folder of src dir
					...
					break;
				case 12:    //update Searched list                                       
                    //((BaseAdapter) lv.getAdapter()).notifyDataSetChanged();
                    if(lv_search_key == null)
						break;
                    ThumbnailView.setAdapter(getSearchedFileListAdapterSorted(cur_path, lv_sort_flag,lv_search_key));                    
                    mSearchedListLoaded = false;
                    if (load_dialog != null)
                        load_dialog.dismiss();
      ...
      /*  ThumbnailView.setOnItemLongClickListener(new OnItemLongClickListener() {
        ...
        Button btn_thumbsearch = (Button) findViewById(R.id.btn_thumbsearch);  
        btn_thumbsearch.setOnClickListener(new OnClickListener() {
    		public void onClick(View v) {
...
*/
	...
	protected Dialog onCreateDialog(int id){
		...
		case SEARCH_DIALOG_ID:
			View layout_search = inflater.inflate(R.layout.search_dialog_layout,
	    		(ViewGroup) findViewById(R.id.layout_root_search));
	    	
	    	search_dialog = new AlertDialog.Builder(ThumbnailView1.this)   
	    	.setView(layout_search)
    ...
    protected void onPrepareDialog(int id, Dialog dialog) {
		...
		case SEARCH_DIALOG_ID:    		
            ...
            dialog.getWindow().setAttributes(lp);  
            search_et = (EditText) search_dialog.getWindow().findViewById(R.id.editTextSearch); 
			Button btn_ok = (Button) search_dialog.getWindow().findViewById(R.id.sc_buttonOK); 
			Button btn_cancel = (Button) search_dialog.getWindow().findViewById(R.id.sc_buttonCancel);  
			btn_ok.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					lv_search_key = String.valueOf(search_et.getText());
					Log.e(TAG,"-------get Text:"+lv_search_key);
					if(lv_search_key == null)
						return;
            		ThumbnailView.setAdapter(getSearchedFileListAdapterSorted(cur_path, lv_sort_flag,lv_search_key));
    		...
    		public void onClick(View v) 
    		{
    			...
    			if(!name.equals(newFileName))
    			{
	    			...
	    			if(mRenameFile.renameTo(new File(newFileName)))
	    			{
	    				db.deleteAllFileMark(); 
	    				ThumbnailView.setAdapter(getFileListAdapterSorted(cur_path, lv_sort_flag));
						//sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.parse("file://"+ newFileName)));
						sendBroadcast(new Intent(Intent.ACTION_MEDIA_MOUNTED, Uri.parse("file://"+ "/mnt")));
	    			}
	    			else
	    			{
	    				Toast.makeText(ThumbnailView1.this,
	...
	private ThumbnailAdapter1 getSearchedFileListAdapterSorted(String path, String sort_type,String key) {
    	if (path.equals(ROOT_PATH)) {
            return new ThumbnailAdapter1(ThumbnailView1.this,
            		getDeviceListData(),
            		R.layout.gridview_item,        		
            ...
            return new ThumbnailAdapter1(ThumbnailView1.this,
            		getSearchedFileListDataSorted(path, sort_type,key),
            		R.layout.gridview_item,        		
                    new String[]{
            	"item_type",
            	"item_sel",
	...
	private List<Map<String, Object>> getSearchedFileListDataSorted(String path, String sort_type,String key) {
        updatePathShow(path);

        if (!mSearchedListLoaded) {
            mSearchedListLoaded = true; 
            showDialog(LOAD_DIALOG_ID);
            
            final String ppath = path;
            final String ssort_type = sort_type;
			final String kkey = key;
            new Thread("getSearchedFileListDataSortedAsync") {
                @Override
                public void run() {                                       
                    mList = getSearchedFileListDataSortedAsync(ppath, ssort_type,kkey);
	...
	/** getSearchedFileListDataSorted */
	private List<Map<String, Object>> getSearchedFileListDataSortedAsync(String path, String sort_type ,String key) {
    	List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();   
        		...
        		if (file_path.listFiles() != null) {
            		if (file_path.listFiles().length > 0) {
            			for (File file : file_path.listFiles()) {
							...
							if((mFileName.indexOf(key)<0)&&(PinyinConv.cn2py(mFileName).indexOf(key)<0))
								continue;
